...
func ScanMedia(dataToScan []byte, contentType string,
	...
	if !(filetype.IsAudio(dataToScan) || filetype.IsVideo(dataToScan) || filetype.IsImage(dataToScan) || isImage(contentType) || isVideo(contentType)) {
		return false, ProxyActionFilterNone
	}
	...
	dataToWrite := dataToScan
	action := ProxyActionFilterNone
	if contentFilterData.FilterResponseAction == ProxyActionBlockedMediaContent {
		action = ProxyActionBlockedMediaContent
		reasonForBlockArray := blockedMediaReasons(contentFilterData.FilterResponse)
		dataToWrite, _ = createEmptyImage(500, 500, "jpeg", reasonForBlockArray)
		resp.StatusCode = http.StatusOK
		stripReplacementRepresentationHeaders(resp.Header)
		resp.Header.Set("Content-Type", "image/jpeg")
		resp.Header.Del("Content-Encoding")
	}

	w.Header().Set("Content-Length", strconv.Itoa(len(dataToWrite)))
	copyResponseHeader(w, resp)
	dest := &accountingWriter{writer: w, contentType: contentType, passthru: passthru}
	_, _ = io.Copy(dest, bytes.NewReader(dataToWrite))
	if DebugLogging {
...
func blockedMediaReasons(response []byte) []string {
	var reasons []string
	if err := json.Unmarshal(response, &reasons); err == nil {
		return append([]string{"", "Image blocked by Gatesentry", "Reason(s) for blocking"}, reasons...)
	}
	reason := strings.TrimSpace(string(response))
	if reason == "" {
		reason = "Blocked by image policy"
	}
	return []string{"", "Image blocked by Gatesentry", "Reason for blocking", reason}
}
