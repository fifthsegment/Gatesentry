<script lang="ts">
    ...
    import { Button, DataTable, FormLabel, TextInput, Toolbar, ToolbarContent, ToolbarSearch } from "carbon-components-svelte";
  ...
  import { notificationstore } from '../../store/notifications';
    ...
    const saveAPIData = () => {
        $store.api.doCall(url, "post", data).then(function (json) {
            if (json.ok == true) {
                loadAPIData();
            } 
        })
...
<Toolbar size="sm">
  <ToolbarContent>
    {#if showForm}
            <TextInput type="text" bind:value={domainText} placeholder="Domain" size="sm"/>
            <TextInput type="text" bind:value={ipText} placeholder="IP" size="sm" />
