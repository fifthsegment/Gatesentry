...
"$ROOT/scripts/check-toolchains.sh" go
...
build() {
  ...
  (cd "$ROOT" && CGO_ENABLED=0 GOOS="$goos" GOARCH="$goarch" GOTOOLCHAIN=go1.24.10 go build -buildvcs=false -trimpath -ldflags="-s -w" -o "$OUTPUT_DIR/$output" .)
}
