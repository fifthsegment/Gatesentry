...
command -v node >/dev/null 2>&1 || { echo "error: Node.js $required_node is required" >&2; exit 1; }
command -v yarn >/dev/null 2>&1 || { echo "error: Yarn $required_yarn is required" >&2; exit 1; }
actual_node=$(node --version)
actual_yarn=$(yarn --version)
[[ "$actual_node" == "$required_node" ]] || { echo "error: Node.js $required_node is required, found $actual_node" >&2; exit 1; }
